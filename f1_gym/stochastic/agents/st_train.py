...
from f1_gym.stochastic.env.st_dynamics import SOFT, MEDIUM, HARD
import pandas as pd
import numpy as np
...
def train(start_compound, compound_name):
    ...
    # Log output folder
    LOG_DIR = "f1_gym/stochastic/dqn_logs"
    MODEL_DIR = "f1_gym/stochastic/dqn_models"
    ...
    model = DQN(
        ...
        tensorboard_log=f"f1_gym/stochastic/dqn_logs/tensorboard/{run.id}"
    )

    # WandB Callback
    callback = WandbCallback(
        model_save_path="f1_gym/stochastic/dqn_models/wandb/{run.id}",
        verbose=2
    )
